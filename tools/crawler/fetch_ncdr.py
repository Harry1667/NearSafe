...
# v1 備援路徑的中文白名單：直接從主路徑代碼表衍生，兩條路永遠同一套類別。
# 2026-07-18 前這裡是手寫的 13 類舊清單——主路徑（官方 API）放寬後備援沒跟上，
# 官方 API 一掛、退回公開 feed 時就會默默縮回窄版類別。改成衍生寫法讓漂移不再可能。
RELEVANT_CATEGORIES = {name for name, _group in RELEVANT_CAPCODES.values()}
